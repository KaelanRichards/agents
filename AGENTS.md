...
- Homebrew at `/opt/homebrew`. Editor: **Zed** (`zed`). Terminal: **Ghostty**.
  Multiplexer: **herdr**. Containers: **OrbStack** (`orb` / `orbctl`).

## The machine is declared, not installed — READ BEFORE INSTALLING ANYTHING
- **Everything installed lives in `~/.config/agents/nix/`.** `darwin.nix` holds the macOS
  settings and the full Homebrew list; `home.nix` holds the config symlinks; `flake.nix`
  pins nixpkgs, nix-darwin, home-manager, and nix-homebrew to stable **26.05**. Apply with
  **`bin/rebuild`**, which needs the user's sudo password — ask them to run it.
- **Never `brew install` ad hoc.** Add the package to `brews` or `casks` in `darwin.nix`,
  then rebuild. Homebrew activation runs `cleanup = "zap"`, so anything installed by hand
  and not written down is **deleted on the next rebuild**. This is the point: it makes drift
  impossible rather than merely discouraged.
- **Config files are symlinks into this repo** (ghostty, zed, mise, herdr, jj). A tool that
  rewrites its own config at runtime writes back into the repo, so the change is versioned.
...
- **The Mac and the Linux VM are provisioned separately.** `nix/darwin.nix` is the Mac;
  the root `Brewfile` + `bootstrap.sh` is the VM, which still uses tmux. nix-darwin's
  homebrew module does not run on Linux, so a tool wanted on both must be added twice.

## Languages & version management
- **Node/TypeScript, Python, pnpm** are managed by **mise** (`config/mise/config.toml`).
  Pin versions with `mise use [-g] <tool>@<version>`. mise shims are on PATH, so
  `node`/`npx`/`python` resolve in scripts too.
- **mise is deliberately kept alongside Nix.** Nix pins one version per machine; mise reads
  `.nvmrc` / `.node-version` per repo, which is what working across repos needs. Nix declares
  mise itself; mise picks the runtime. Do not try to move node/python into Nix.
...
- **Do not give much weight to development cost.** When choosing between approaches, weigh
  quality, simplicity, robustness, scalability, and long-term maintainability instead. You
  learned cost estimates from humans who think in days and weeks, so you tend to over-value
  the cheap, narrow fix. Price the work at what it costs *you*, then pick the better design.
- **Start every bug fix by reproducing the bug end to end**, as close to how a user hits it
  as you can get. Do not reason your way to a cause and patch that. A fix for a problem you
  never saw fail is a guess.
- **Fix what you pass on the way.** A failing lint rule, a flaky test, or an obviously broken
  bit of UI gets fixed even when it has nothing to do with the current task. Say in the final
  report what you fixed and why, so the extra diff is never a surprise.
- **Never hand-edit generated files.** Lockfiles, codegen output, schema dumps, and anything
  carrying an autogenerated banner change only by re-running the tool that writes them.
...
- **Overview**: `agents-status` — read-only single pane (VMs + cost, health, MCP servers,
  repo/CI + open PRs, herdr sessions and per-agent state).
